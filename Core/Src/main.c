...
//  drv_write_CSACR(drv, 0x0, 0x1, 0x0, CSA_GAIN_40, 0x0, 0x1, 0x1, 0x1, SEN_LVL_0_25);
  ...
  uint16_t value1 = drv_read_register(drv, DCR);

  /* Start the FSM */
  state.state = MENU_MODE;
  state.next_state = ENTER_MODE;
  state.ready = 1;
  ...
  while (1)
  {
	  ...
	  //printf("angle: %.3f\r\n", comm_encoder.angle_singleturn);

	  printf("A,%.3f,%.3f,%.3f\r\n", comm_encoder.angle_singleturn, controller.dtheta_mech, controller.t_ff); // Print angle, velocity, and torque
	  //printf("A,%.3f,%.3f,%.3f,%.3f,%.3f\r\n", comm_encoder.angle_singleturn, controller.dtheta_mech, controller.i_q_des, controller.kp, controller.kd);
