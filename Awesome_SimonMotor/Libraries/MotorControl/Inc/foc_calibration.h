...
#include <stdint.h>
#include "hw_config.h"   /* MEASUREMENTS_PER_POLE_PAIR, PPAIRS_MAX, W_CAL */
#include "foc.h"         /* foc_t */
...
#define MAX_MEASUREMENTS_FOR_ENTIRE_LUT  ((uint32_t)(MEASUREMENTS_PER_POLE_PAIR) * (uint32_t)(PPAIRS_MAX))
...
typedef struct {
    ...
    /* LUT sweep */
    float       all_measurements_for_lookup_table[MAX_MEASUREMENTS_FOR_ENTIRE_LUT]; // CW/CCW-averaged raw error samples
    uint16_t    num_measurements_to_take_for_lut;              // actual samples = detected_ppairs * MEASUREMENTS_PER_POLE_PAIR
